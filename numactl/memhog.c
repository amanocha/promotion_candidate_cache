...
int main(int ac, char **av)
{
	...
	align_addr = ((unsigned long) map)/pagesize * pagesize;
	printf("addr = %lu, aligned addr = %lu\n", (unsigned long) map, align_addr);
	//ret = madvise((void*) align_addr, length, MADV_HUGEPAGE);
	//if (ret != 0) terr("madvise");

	if (mlock((char*) align_addr, length-pagesize)) {
		terr("mlock");
	}
