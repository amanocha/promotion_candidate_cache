...
def main():
  global apps, is_thp, RESULT_DIR, inputs, datasets, vp_inputs
  ...
  if args.experiment == 0:
    ...
    for i in range(NUM_PERCENTAGES):
      percentage = int(math.pow(2, i))
      run_two("cache", percentage, PCC_SIZE, access_time=ACCESS_TIME, promote_all=True)
    run_two("cache", 100, PCC_SIZE, access_time=ACCESS_TIME, promote_all=True)
    ...
    run_two("cache", 100, PCC_SIZE, demotion=True)
