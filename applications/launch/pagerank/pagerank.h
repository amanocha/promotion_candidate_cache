...
void init_pagerank(int run_kernel, csr_graph G, double **x, double **in_r, double **ret, unsigned long **in_wl, unsigned long *in_index, unsigned long **out_wl, unsigned long *out_index) {
  ...
  *in_r = static_cast<double*>(tmp);
  ...
  if (run_kernel == MADVISE_ALL) {
    err = madvise(*in_wl, G.nodes * sizeof(unsigned long), MADV_HUGEPAGE);
    if (err != 0) perror("Error!");
    else cout << "MADV_HUGEPAGE in worklist successful!" << endl;

    err = madvise(*out_wl, G.nodes * sizeof(unsigned long), MADV_HUGEPAGE);
    if (err != 0) perror("Error!");
    else cout << "MADV_HUGEPAGE out worklist successful!" << endl;

    err = madvise(*x, G.nodes * sizeof(double), MADV_HUGEPAGE);
    if (err != 0) perror("Error!");
    else cout << "MADV_HUGEPAGE x successful!" << endl;

    err = madvise(*in_r, G.nodes * sizeof(double), MADV_HUGEPAGE);
    if (err != 0) perror("Error!");
    else cout << "MADV_HUGEPAGE in_r successful!" << endl;
  }
